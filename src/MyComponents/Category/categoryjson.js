// older Obj
let categoryData ={
  "Veg-Masala-Tadka": {
    "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
    "items": [
      {
        "cardId": 1,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
        "title": "Pizza Hut",
        "type": "Pizzas, Italian",
        "mealDetail": "Delicious hand-tossed pizzas with a variety of toppings.",
        "ingredients": [
          "Wheat flour",
          "Cheese",
          "Tomato sauce",
          "Olive oil",
          "Vegetables"
        ]
      },
      {
        "cardId": 2,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/noc7ieivirqxtpujhsgl",
        "title": "UBQ by Barbeque",
        "type": "North Indian, Barbeque",
        "mealDetail": "Succulent barbeque dishes with authentic North Indian spices.",
        "ingredients": [
          "Chicken",
          "Yogurt",
          "Spices",
          "Vegetables",
          "Barbeque sauce"
        ]
      },
      {
        "cardId": 3,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/iydxtay1mnne2ktw7txe",
        "title": "Barbeque Nation",
        "type": "North Indian, Barbeque",
        "mealDetail": "Wide range of barbeque options with a buffet style serving.",
        "ingredients": [
          "Meat",
          "Paneer",
          "Spices",
          "Vegetables",
          "Chutneys"
        ]
      },
      {
        "cardId": 4,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        "title": "Burger King",
        "type": "Burgers, American",
        "mealDetail": "Classic American burgers with a variety of fillings.",
        "ingredients": [
          "Beef",
          "Lettuce",
          "Tomato",
          "Cheese",
          "Buns"
        ]
      },
      {
        "cardId": 5,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/91afd7c438fcf1a3b55b97c2fbaa1cfa",
        "title": "Radha Krishna Puri",
        "type": "North Indian, Chinese",
        "mealDetail": "Authentic North Indian and Chinese dishes.",
        "ingredients": [
          "Paneer",
          "Capsicum",
          "Soy sauce",
          "Rice",
          "Spices"
        ]
      },
      {
        "cardId": 6,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/69a061b7e0f951cef2b4947946f94ec6",
        "title": "Box8 - Desi Meals",
        "type": "North Indian, Biryani, Thalis",
        "mealDetail": "Traditional desi meals with a variety of dishes.",
        "ingredients": [
          "Rice",
          "Chicken",
          "Spices",
          "Vegetables",
          "Yogurt"
        ]
      },
      {
        "cardId": 7,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        "title": "Green Delight",
        "type": "Salads, Healthy",
        "mealDetail": "Fresh and healthy salads with a variety of dressings.",
        "ingredients": [
          "Lettuce",
          "Tomatoes",
          "Cucumbers",
          "Olive oil",
          "Herbs"
        ]
      },
      {
        "cardId": 8,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
        "title": "Veggie Feast",
        "type": "Vegetarian, Continental",
        "mealDetail": "Delicious vegetarian dishes with a continental twist.",
        "ingredients": [
          "Paneer",
          "Bell peppers",
          "Tomatoes",
          "Olive oil",
          "Herbs"
        ]
      }
    ]
  },
  "Non-Veg": {
    "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/63178e3e64d503a479f2a2048a474552",
    "items": [
      {
        "cardId": 3,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/63178e3e64d503a479f2a2048a474552",
        "title": "KFC",
        "type": "Fried Chicken, Fast Food",
        "mealDetail": "Crispy and juicy fried chicken with a secret blend of herbs and spices.",
        "ingredients": [
          "Chicken",
          "Flour",
          "Spices",
          "Oil",
          "Buttermilk"
        ]
      },
      {
        "cardId": 4,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/RX_THUMBNAIL/IMAGES/VENDOR/2024/5/20/349e8d46-3138-4b19-96b2-df400a77e285_622197.JPG",
        "title": "Domino's Pizza",
        "type": "Pizzas, Italian",
        "mealDetail": "Hot and fresh pizzas with a variety of toppings and crust options.",
        "ingredients": [
          "Wheat flour",
          "Cheese",
          "Tomato sauce",
          "Pepperoni",
          "Vegetables"
        ]
      },
      {
        "cardId": 5,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/85ccae4e3576f9330af102c46ca85395",
        "title": "Grill House",
        "type": "Grilled, Barbeque",
        "mealDetail": "Grilled meats and vegetables with a smoky flavor.",
        "ingredients": [
          "Chicken",
          "Beef",
          "Vegetables",
          "Spices",
          "Grill sauce"
        ]
      },
      {
        "cardId": 6,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/noc7ieivirqxtpujhsgl",
        "title": "Spicy Chicken",
        "type": "Indian, Fast Food",
        "mealDetail": "Spicy and flavorful chicken dishes with Indian spices.",
        "ingredients": [
          "Chicken",
          "Spices",
          "Oil",
          "Yogurt",
          "Herbs"
        ]
      },
      {
        "cardId": 7,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        "title": "Chicken Delight",
        "type": "Chicken, Indian",
        "mealDetail": "Delicious chicken dishes with Indian spices.",
        "ingredients": [
          "Chicken",
          "Yogurt",
          "Spices",
          "Onions",
          "Tomatoes"
        ]
      }
    ]
  },
  "Curry": {
    "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/69a061b7e0f951cef2b4947946f94ec6",
    "items": [
      {
        "cardId": 5,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/RX_THUMBNAIL/IMAGES/VENDOR/2024/4/1/b5c7e325-a2b3-4334-b104-0b20df81dd93_23728.JPG",
        "title": "Curry House",
        "type": "Indian, Curry",
        "mealDetail": "A variety of Indian curries with rich and flavorful gravies.",
        "ingredients": [
          "Spices",
          "Tomatoes",
          "Onions",
          "Yogurt",
          "Herbs"
        ]
      },
      {
        "cardId": 6,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        "title": "Veg Curry",
        "type": "Vegetarian, Indian",
        "mealDetail": "Delicious vegetarian curries with rich flavors.",
        "ingredients": [
          "Paneer",
          "Tomatoes",
          "Onions",
          "Spices",
          "Herbs"
        ]
      },
      {
        "cardId": 7,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/RX_THUMBNAIL/IMAGES/VENDOR/2024/5/20/349e8d46-3138-4b19-96b2-df400a77e285_622197.JPG",
        "title": "Chicken Curry",
        "type": "Indian, Curry",
        "mealDetail": "Spicy and flavorful chicken curry with a rich gravy.",
        "ingredients": [
          "Chicken",
          "Tomatoes",
          "Onions",
          "Spices",
          "Coconut milk"
        ]
      },
      {
        "cardId": 8,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/6e44fd7f1e5cd9967edfe47c10247671",
        "title": "Fish Curry",
        "type": "Indian, Seafood",
        "mealDetail": "Traditional fish curry with aromatic spices and a tangy gravy.",
        "ingredients": [
          "Fish",
          "Spices",
          "Coconut milk",
          "Tamarind",
          "Curry leaves"
        ]
      }
    ]
  },
  "Lunch": {
    "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
    "items": [
      {
        "cardId": 7,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/RX_THUMBNAIL/IMAGES/VENDOR/2024/5/19/741d7d41-1341-4358-b6e0-cc22b8e82f9a_750391.JPG",
        "title": "Lunch Box",
        "type": "Indian, Meals",
        "mealDetail": "Convenient and nutritious lunch options with balanced meals.",
        "ingredients": [
          "Rice",
          "Dal",
          "Vegetables",
          "Roti",
          "Salad"
        ]
      },
      {
        "cardId": 8,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/6e44fd7f1e5cd9967edfe47c10247671",
        "title": "Healthy Lunch",
        "type": "Healthy, Indian",
        "mealDetail": "Wholesome and healthy lunch options for a balanced diet.",
        "ingredients": [
          "Quinoa",
          "Grilled chicken",
          "Vegetables",
          "Olive oil",
          "Herbs"
        ]
      },
      {
        "cardId": 9,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
        "title": "Quick Lunch",
        "type": "Indian, Fast Food",
        "mealDetail": "Quick and tasty lunch options for busy schedules.",
        "ingredients": [
          "Rice",
          "Paneer",
          "Vegetables",
          "Spices",
          "Chutneys"
        ]
      }
    ]
  },
  "Chicken": {
    "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/85ccae4e3576f9330af102c46ca85395",
    "items": [
      {
        "cardId": 9,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/69a061b7e0f951cef2b4947946f94ec6",
        "title": "Chicken Delight",
        "type": "Chicken, Indian",
        "mealDetail": "Delicious chicken dishes with Indian spices.",
        "ingredients": [
          "Chicken",
          "Yogurt",
          "Spices",
          "Onions",
          "Tomatoes"
        ]
      },
      {
        "cardId": 10,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/85ccae4e3576f9330af102c46ca85395",
        "title": "Chicken Curry",
        "type": "Indian, Curry",
        "mealDetail": "Spicy and flavorful chicken curry with a rich gravy.",
        "ingredients": [
          "Chicken",
          "Tomatoes",
          "Onions",
          "Spices",
          "Coconut milk"
        ]
      },
      {
        "cardId": 11,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        "title": "Chicken Biryani",
        "type": "Indian, Biryani",
        "mealDetail": "Fragrant and flavorful chicken biryani with aromatic spices.",
        "ingredients": [
          "Rice",
          "Chicken",
          "Spices",
          "Yogurt",
          "Fried onions"
        ]
      },
      {
        "cardId": 5,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/RX_THUMBNAIL/IMAGES/VENDOR/2024/4/1/b5c7e325-a2b3-4334-b104-0b20df81dd93_23728.JPG",
        "title": "Curry House",
        "type": "Indian, Curry",
        "mealDetail": "A variety of Indian curries with rich and flavorful gravies.",
        "ingredients": [
          "Spices",
          "Tomatoes",
          "Onions",
          "Yogurt",
          "Herbs"
        ]
      },
      {
        "cardId": 6,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        "title": "Veg Curry",
        "type": "Vegetarian, Indian",
        "mealDetail": "Delicious vegetarian curries with rich flavors.",
        "ingredients": [
          "Paneer",
          "Tomatoes",
          "Onions",
          "Spices",
          "Herbs"
        ]
      },
      {
        "cardId": 7,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/RX_THUMBNAIL/IMAGES/VENDOR/2024/5/20/349e8d46-3138-4b19-96b2-df400a77e285_622197.JPG",
        "title": "Chicken Curry",
        "type": "Indian, Curry",
        "mealDetail": "Spicy and flavorful chicken curry with a rich gravy.",
        "ingredients": [
          "Chicken",
          "Tomatoes",
          "Onions",
          "Spices",
          "Coconut milk"
        ]
      },
      {
        "cardId": 8,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/6e44fd7f1e5cd9967edfe47c10247671",
        "title": "Fish Curry",
        "type": "Indian, Seafood",
        "mealDetail": "Traditional fish curry with aromatic spices and a tangy gravy.",
        "ingredients": [
          "Fish",
          "Spices",
          "Coconut milk",
          "Tamarind",
          "Curry leaves"
        ]
      }
    ]
  },
  "Rice": {
    "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
    "items": [
      {
        "cardId": 12,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/91afd7c438fcf1a3b55b97c2fbaa1cfa",
        "title": "Veg Biryani",
        "type": "Indian, Biryani",
        "mealDetail": "Aromatic and flavorful veg biryani with spices and vegetables.",
        "ingredients": [
          "Rice",
          "Vegetables",
          "Spices",
          "Yogurt",
          "Herbs"
        ]
      },
      {
        "cardId": 13,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/69a061b7e0f951cef2b4947946f94ec6",
        "title": "Chicken Biryani",
        "type": "Indian, Biryani",
        "mealDetail": "Fragrant and flavorful chicken biryani with aromatic spices.",
        "ingredients": [
          "Rice",
          "Chicken",
          "Spices",
          "Yogurt",
          "Fried onions"
        ]
      },
      {
        "cardId": 14,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        "title": "Hyderabadi Biryani",
        "type": "Indian, Biryani",
        "mealDetail": "Traditional Hyderabadi biryani with rich flavors and spices.",
        "ingredients": [
          "Rice",
          "Meat",
          "Spices",
          "Yogurt",
          "Fried onions"
        ]
      }
    ]
  },
  "Paneer": {
    "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
    "items": [
      {
        "cardId": 15,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
        "title": "Paneer Tikka",
        "type": "Indian, Barbeque",
        "mealDetail": "Marinated paneer grilled to perfection with spices.",
        "ingredients": [
          "Paneer",
          "Yogurt",
          "Spices",
          "Capsicum",
          "Onions"
        ]
      },
      {
        "cardId": 16,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/noc7ieivirqxtpujhsgl",
        "title": "Paneer Butter Masala",
        "type": "Indian, Curry",
        "mealDetail": "Creamy and rich paneer butter masala with a buttery gravy.",
        "ingredients": [
          "Paneer",
          "Tomatoes",
          "Butter",
          "Cream",
          "Spices"
        ]
      },
      {
        "cardId": 17,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/85ccae4e3576f9330af102c46ca85395",
        "title": "Paneer Biryani",
        "type": "Indian, Biryani",
        "mealDetail": "Flavorful paneer biryani with aromatic spices.",
        "ingredients": [
          "Rice",
          "Paneer",
          "Spices",
          "Yogurt",
          "Herbs"
        ]
      }
    ]
  },
  "Desserts": {
    "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/85ccae4e3576f9330af102c46ca85395",
    "items": [
      {
        "cardId": 18,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/63178e3e64d503a479f2a2048a474552",
        "title": "Chocolate Cake",
        "type": "Desserts, Bakery",
        "mealDetail": "Rich and moist chocolate cake with layers of creamy frosting.",
        "ingredients": [
          "Flour",
          "Cocoa",
          "Sugar",
          "Butter",
          "Eggs"
        ]
      },
      {
        "cardId": 19,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/85ccae4e3576f9330af102c46ca85395",
        "title": "Ice Cream",
        "type": "Desserts, Frozen",
        "mealDetail": "Creamy and delicious ice cream with a variety of flavors.",
        "ingredients": [
          "Milk",
          "Cream",
          "Sugar",
          "Flavors",
          "Nuts"
        ]
      },
      {
        "cardId": 20,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
        "title": "Gulab Jamun",
        "type": "Desserts, Indian",
        "mealDetail": "Soft and spongy gulab jamun soaked in sweet syrup.",
        "ingredients": [
          "Milk solids",
          "Flour",
          "Sugar",
          "Cardamom",
          "Rose water"
        ]
      }
    ]
  },
  "Healthy": {
    "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/69a061b7e0f951cef2b4947946f94ec6",
    "items": [
      {
        "cardId": 21,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        "title": "Green Salad",
        "type": "Salads, Healthy",
        "mealDetail": "Fresh and nutritious green salad with a variety of vegetables.",
        "ingredients": [
          "Lettuce",
          "Cucumbers",
          "Tomatoes",
          "Olive oil",
          "Lemon juice"
        ]
      },
      {
        "cardId": 22,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/85ccae4e3576f9330af102c46ca85395",
        "title": "Fruit Salad",
        "type": "Salads, Healthy",
        "mealDetail": "Refreshing fruit salad with seasonal fruits.",
        "ingredients": [
          "Apples",
          "Bananas",
          "Oranges",
          "Grapes",
          "Mint"
        ]
      },
      {
        "cardId": 23,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
        "title": "Smoothie Bowl",
        "type": "Healthy, Breakfast",
        "mealDetail": "Delicious smoothie bowl topped with fresh fruits and nuts.",
        "ingredients": [
          "Berries",
          "Banana",
          "Yogurt",
          "Nuts",
          "Honey"
        ]
      },
      {
        "cardId": 3,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/iydxtay1mnne2ktw7txe",
        "title": "Barbeque Nation",
        "type": "North Indian, Barbeque",
        "mealDetail": "Wide range of barbeque options with a buffet style serving.",
        "ingredients": [
          "Meat",
          "Paneer",
          "Spices",
          "Vegetables",
          "Chutneys"
        ]
      },
      {
        "cardId": 4,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        "title": "Burger King",
        "type": "Burgers, American",
        "mealDetail": "Classic American burgers with a variety of fillings.",
        "ingredients": [
          "Beef",
          "Lettuce",
          "Tomato",
          "Cheese",
          "Buns"
        ]
      },
      {
        "cardId": 5,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/91afd7c438fcf1a3b55b97c2fbaa1cfa",
        "title": "Radha Krishna Puri",
        "type": "North Indian, Chinese",
        "mealDetail": "Authentic North Indian and Chinese dishes.",
        "ingredients": [
          "Paneer",
          "Capsicum",
          "Soy sauce",
          "Rice",
          "Spices"
        ]
      },
      {
        "cardId": 6,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/69a061b7e0f951cef2b4947946f94ec6",
        "title": "Box8 - Desi Meals",
        "type": "North Indian, Biryani, Thalis",
        "mealDetail": "Traditional desi meals with a variety of dishes.",
        "ingredients": [
          "Rice",
          "Chicken",
          "Spices",
          "Vegetables",
          "Yogurt"
        ]
      },
      {
        "cardId": 7,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        "title": "Green Delight",
        "type": "Salads, Healthy",
        "mealDetail": "Fresh and healthy salads with a variety of dressings.",
        "ingredients": [
          "Lettuce",
          "Tomatoes",
          "Cucumbers",
          "Olive oil",
          "Herbs"
        ]
      },
      {
        "cardId": 8,
        "imageId": "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
        "title": "Veggie Feast",
        "type": "Vegetarian, Continental",
        "mealDetail": "Delicious vegetarian dishes with a continental twist.",
        "ingredients": [
          "Paneer",
          "Bell peppers",
          "Tomatoes",
          "Olive oil",
          "Herbs"
        ]
      }
    ]
  }
};

//new Obj
let categoryData2 = "https://mocki.io/v1/47d919d8-529a-466e-bbc7-8ce905b7a39a";

let lasmainobj = "https://mocki.io/v1/496c738c-f995-4bc4-aace-abf2370fadd1";

let newarrobj = "https://mocki.io/v1/a8e21ecb-f9ad-4d37-97ae-9f124e564962";

let newobj2 = {
  "Veg-Masala-Tadka": {
    imageId:
      "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
    items: [
      {
        cardId: 1,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
        title: "Pizza Hut",
        type: "Pizzas, Italian",
        mealDetail: "Delicious hand-tossed pizzas with a variety of toppings.",
        ingredients: [
          "Wheat flour",
          "Cheese",
          "Tomato sauce",
          "Olive oil",
          "Vegetables",
        ],
      },
      {
        cardId: 2,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/noc7ieivirqxtpujhsgl",
        title: "UBQ by Barbeque",
        type: "North Indian, Barbeque",
        mealDetail:
          "Succulent barbeque dishes with authentic North Indian spices.",
        ingredients: [
          "Chicken",
          "Yogurt",
          "Spices",
          "Vegetables",
          "Barbeque sauce",
        ],
      },
      {
        cardId: 3,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/iydxtay1mnne2ktw7txe",
        title: "Barbeque Nation",
        type: "North Indian, Barbeque",
        mealDetail:
          "Wide range of barbeque options with a buffet style serving.",
        ingredients: ["Meat", "Paneer", "Spices", "Vegetables", "Chutneys"],
      },
      {
        cardId: 4,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        title: "Burger King",
        type: "Burgers, American",
        mealDetail: "Classic American burgers with a variety of fillings.",
        ingredients: ["Beef", "Lettuce", "Tomato", "Cheese", "Buns"],
      },
      {
        cardId: 5,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/91afd7c438fcf1a3b55b97c2fbaa1cfa",
        title: "Radha Krishna Puri",
        type: "North Indian, Chinese",
        mealDetail: "Authentic North Indian and Chinese dishes.",
        ingredients: ["Paneer", "Capsicum", "Soy sauce", "Rice", "Spices"],
      },
      {
        cardId: 6,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/69a061b7e0f951cef2b4947946f94ec6",
        title: "Box8 - Desi Meals",
        type: "North Indian, Biryani, Thalis",
        mealDetail: "Traditional desi meals with a variety of dishes.",
        ingredients: ["Rice", "Chicken", "Spices", "Vegetables", "Yogurt"],
      },
      {
        cardId: 7,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        title: "Green Delight",
        type: "Salads, Healthy",
        mealDetail: "Fresh and healthy salads with a variety of dressings.",
        ingredients: ["Lettuce", "Tomatoes", "Cucumbers", "Olive oil", "Herbs"],
      },
      {
        cardId: 8,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
        title: "Veggie Feast",
        type: "Vegetarian, Continental",
        mealDetail: "Delicious vegetarian dishes with a continental twist.",
        ingredients: [
          "Paneer",
          "Bell peppers",
          "Tomatoes",
          "Olive oil",
          "Herbs",
        ],
      },
    ],
  },
  "Non-Veg": {
    imageId:
      "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/63178e3e64d503a479f2a2048a474552",
    items: [
      {
        cardId: 3,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/63178e3e64d503a479f2a2048a474552",
        title: "KFC",
        type: "Fried Chicken, Fast Food",
        mealDetail:
          "Crispy and juicy fried chicken with a secret blend of herbs and spices.",
        ingredients: ["Chicken", "Flour", "Spices", "Oil", "Buttermilk"],
      },
      {
        cardId: 4,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/RX_THUMBNAIL/IMAGES/VENDOR/2024/5/20/349e8d46-3138-4b19-96b2-df400a77e285_622197.JPG",
        title: "Domino's Pizza",
        type: "Pizzas, Italian",
        mealDetail:
          "Hot and fresh pizzas with a variety of toppings and crust options.",
        ingredients: [
          "Wheat flour",
          "Cheese",
          "Tomato sauce",
          "Pepperoni",
          "Vegetables",
        ],
      },
      {
        cardId: 5,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/85ccae4e3576f9330af102c46ca85395",
        title: "Grill House",
        type: "Grilled, Barbeque",
        mealDetail: "Grilled meats and vegetables with a smoky flavor.",
        ingredients: ["Chicken", "Beef", "Vegetables", "Spices", "Grill sauce"],
      },
      {
        cardId: 6,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/noc7ieivirqxtpujhsgl",
        title: "Spicy Chicken",
        type: "Indian, Fast Food",
        mealDetail: "Spicy and flavorful chicken dishes with Indian spices.",
        ingredients: ["Chicken", "Spices", "Oil", "Yogurt", "Herbs"],
      },
      {
        cardId: 7,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        title: "Chicken Delight",
        type: "Chicken, Indian",
        mealDetail: "Delicious chicken dishes with Indian spices.",
        ingredients: ["Chicken", "Yogurt", "Spices", "Onions", "Tomatoes"],
      },
    ],
  },
  "Curry": {
    imageId:
      "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/69a061b7e0f951cef2b4947946f94ec6",
    items: [
      {
        cardId: 5,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/RX_THUMBNAIL/IMAGES/VENDOR/2024/4/1/b5c7e325-a2b3-4334-b104-0b20df81dd93_23728.JPG",
        title: "Curry House",
        type: "Indian, Curry",
        mealDetail:
          "A variety of Indian curries with rich and flavorful gravies.",
        ingredients: ["Spices", "Tomatoes", "Onions", "Yogurt", "Herbs"],
      },
      {
        cardId: 6,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        title: "Veg Curry",
        type: "Vegetarian, Indian",
        mealDetail: "Delicious vegetarian curries with rich flavors.",
        ingredients: ["Paneer", "Tomatoes", "Onions", "Spices", "Herbs"],
      },
      {
        cardId: 7,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/RX_THUMBNAIL/IMAGES/VENDOR/2024/5/20/349e8d46-3138-4b19-96b2-df400a77e285_622197.JPG",
        title: "Chicken Curry",
        type: "Indian, Curry",
        mealDetail: "Spicy and flavorful chicken curry with a rich gravy.",
        ingredients: [
          "Chicken",
          "Tomatoes",
          "Onions",
          "Spices",
          "Coconut milk",
        ],
      },
      {
        cardId: 8,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/6e44fd7f1e5cd9967edfe47c10247671",
        title: "Fish Curry",
        type: "Indian, Seafood",
        mealDetail:
          "Traditional fish curry with aromatic spices and a tangy gravy.",
        ingredients: [
          "Fish",
          "Spices",
          "Coconut milk",
          "Tamarind",
          "Curry leaves",
        ],
      },
    ],
  },
  "Lunch": {
    imageId:
      "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
    items: [
      {
        cardId: 7,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/RX_THUMBNAIL/IMAGES/VENDOR/2024/5/19/741d7d41-1341-4358-b6e0-cc22b8e82f9a_750391.JPG",
        title: "Lunch Box",
        type: "Indian, Meals",
        mealDetail:
          "Convenient and nutritious lunch options with balanced meals.",
        ingredients: ["Rice", "Dal", "Vegetables", "Roti", "Salad"],
      },
      {
        cardId: 8,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/6e44fd7f1e5cd9967edfe47c10247671",
        title: "Healthy Lunch",
        type: "Healthy, Indian",
        mealDetail: "Wholesome and healthy lunch options for a balanced diet.",
        ingredients: [
          "Quinoa",
          "Grilled chicken",
          "Vegetables",
          "Olive oil",
          "Herbs",
        ],
      },
      {
        cardId: 9,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
        title: "Quick Lunch",
        type: "Indian, Fast Food",
        mealDetail: "Quick and tasty lunch options for busy schedules.",
        ingredients: ["Rice", "Paneer", "Vegetables", "Spices", "Chutneys"],
      },
    ],
  },
  Chicken: {
    imageId:
      "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/85ccae4e3576f9330af102c46ca85395",
    items: [
      {
        cardId: 9,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/69a061b7e0f951cef2b4947946f94ec6",
        title: "Chicken Delight",
        type: "Chicken, Indian",
        mealDetail: "Delicious chicken dishes with Indian spices.",
        ingredients: ["Chicken", "Yogurt", "Spices", "Onions", "Tomatoes"],
      },
      {
        cardId: 10,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/85ccae4e3576f9330af102c46ca85395",
        title: "Chicken Curry",
        type: "Indian, Curry",
        mealDetail: "Spicy and flavorful chicken curry with a rich gravy.",
        ingredients: [
          "Chicken",
          "Tomatoes",
          "Onions",
          "Spices",
          "Coconut milk",
        ],
      },
      {
        cardId: 11,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        title: "Chicken Biryani",
        type: "Indian, Biryani",
        mealDetail:
          "Fragrant and flavorful chicken biryani with aromatic spices.",
        ingredients: ["Rice", "Chicken", "Spices", "Yogurt", "Fried onions"],
      },
    ],
  },
  Rice: {
    imageId:
      "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
    items: [
      {
        cardId: 12,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/91afd7c438fcf1a3b55b97c2fbaa1cfa",
        title: "Veg Biryani",
        type: "Indian, Biryani",
        mealDetail:
          "Aromatic and flavorful veg biryani with spices and vegetables.",
        ingredients: ["Rice", "Vegetables", "Spices", "Yogurt", "Herbs"],
      },
      {
        cardId: 13,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/69a061b7e0f951cef2b4947946f94ec6",
        title: "Chicken Biryani",
        type: "Indian, Biryani",
        mealDetail:
          "Fragrant and flavorful chicken biryani with aromatic spices.",
        ingredients: ["Rice", "Chicken", "Spices", "Yogurt", "Fried onions"],
      },
      {
        cardId: 14,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        title: "Hyderabadi Biryani",
        type: "Indian, Biryani",
        mealDetail:
          "Traditional Hyderabadi biryani with rich flavors and spices.",
        ingredients: ["Rice", "Meat", "Spices", "Yogurt", "Fried onions"],
      },
    ],
  },
  Paneer: {
    imageId:
      "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
    items: [
      {
        cardId: 15,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
        title: "Paneer Tikka",
        type: "Indian, Barbeque",
        mealDetail: "Marinated paneer grilled to perfection with spices.",
        ingredients: ["Paneer", "Yogurt", "Spices", "Capsicum", "Onions"],
      },
      {
        cardId: 16,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/noc7ieivirqxtpujhsgl",
        title: "Paneer Butter Masala",
        type: "Indian, Curry",
        mealDetail:
          "Creamy and rich paneer butter masala with a buttery gravy.",
        ingredients: ["Paneer", "Tomatoes", "Butter", "Cream", "Spices"],
      },
      {
        cardId: 17,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/85ccae4e3576f9330af102c46ca85395",
        title: "Paneer Biryani",
        type: "Indian, Biryani",
        mealDetail: "Flavorful paneer biryani with aromatic spices.",
        ingredients: ["Rice", "Paneer", "Spices", "Yogurt", "Herbs"],
      },
      {
        cardId: 3,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/63178e3e64d503a479f2a2048a474552",
        title: "KFC",
        type: "Fried Chicken, Fast Food",
        mealDetail:
          "Crispy and juicy fried chicken with a secret blend of herbs and spices.",
        ingredients: ["Chicken", "Flour", "Spices", "Oil", "Buttermilk"],
      },
      {
        cardId: 4,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/RX_THUMBNAIL/IMAGES/VENDOR/2024/5/20/349e8d46-3138-4b19-96b2-df400a77e285_622197.JPG",
        title: "Domino's Pizza",
        type: "Pizzas, Italian",
        mealDetail:
          "Hot and fresh pizzas with a variety of toppings and crust options.",
        ingredients: [
          "Wheat flour",
          "Cheese",
          "Tomato sauce",
          "Pepperoni",
          "Vegetables",
        ],
      },
      {
        cardId: 5,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/85ccae4e3576f9330af102c46ca85395",
        title: "Grill House",
        type: "Grilled, Barbeque",
        mealDetail: "Grilled meats and vegetables with a smoky flavor.",
        ingredients: ["Chicken", "Beef", "Vegetables", "Spices", "Grill sauce"],
      },
      {
        cardId: 6,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/noc7ieivirqxtpujhsgl",
        title: "Spicy Chicken",
        type: "Indian, Fast Food",
        mealDetail: "Spicy and flavorful chicken dishes with Indian spices.",
        ingredients: ["Chicken", "Spices", "Oil", "Yogurt", "Herbs"],
      },
      {
        cardId: 7,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        title: "Chicken Delight",
        type: "Chicken, Indian",
        mealDetail: "Delicious chicken dishes with Indian spices.",
        ingredients: ["Chicken", "Yogurt", "Spices", "Onions", "Tomatoes"],
      },
    ],
  },
  Desserts: {
    imageId:
      "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/85ccae4e3576f9330af102c46ca85395",
    items: [
      {
        cardId: 18,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/63178e3e64d503a479f2a2048a474552",
        title: "Chocolate Cake",
        type: "Desserts, Bakery",
        mealDetail:
          "Rich and moist chocolate cake with layers of creamy frosting.",
        ingredients: ["Flour", "Cocoa", "Sugar", "Butter", "Eggs"],
      },
      {
        cardId: 19,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/85ccae4e3576f9330af102c46ca85395",
        title: "Ice Cream",
        type: "Desserts, Frozen",
        mealDetail: "Creamy and delicious ice cream with a variety of flavors.",
        ingredients: ["Milk", "Cream", "Sugar", "Flavors", "Nuts"],
      },
      {
        cardId: 20,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
        title: "Gulab Jamun",
        type: "Desserts, Indian",
        mealDetail: "Soft and spongy gulab jamun soaked in sweet syrup.",
        ingredients: [
          "Milk solids",
          "Flour",
          "Sugar",
          "Cardamom",
          "Rose water",
        ],
      },
    ],
  },
  Healthy: {
    imageId:
      "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/69a061b7e0f951cef2b4947946f94ec6",
    items: [
      {
        cardId: 21,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        title: "Green Salad",
        type: "Salads, Healthy",
        mealDetail:
          "Fresh and nutritious green salad with a variety of vegetables.",
        ingredients: [
          "Lettuce",
          "Cucumbers",
          "Tomatoes",
          "Olive oil",
          "Lemon juice",
        ],
      },
      {
        cardId: 22,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/85ccae4e3576f9330af102c46ca85395",
        title: "Fruit Salad",
        type: "Salads, Healthy",
        mealDetail: "Refreshing fruit salad with seasonal fruits.",
        ingredients: ["Apples", "Bananas", "Oranges", "Grapes", "Mint"],
      },
      {
        cardId: 23,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
        title: "Smoothie Bowl",
        type: "Healthy, Breakfast",
        mealDetail:
          "Delicious smoothie bowl topped with fresh fruits and nuts.",
        ingredients: ["Berries", "Banana", "Yogurt", "Nuts", "Honey"],
      },
      {
        cardId: 3,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/iydxtay1mnne2ktw7txe",
        title: "Barbeque Nation",
        type: "North Indian, Barbeque",
        mealDetail:
          "Wide range of barbeque options with a buffet style serving.",
        ingredients: ["Meat", "Paneer", "Spices", "Vegetables", "Chutneys"],
      },
      {
        cardId: 4,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        title: "Burger King",
        type: "Burgers, American",
        mealDetail: "Classic American burgers with a variety of fillings.",
        ingredients: ["Beef", "Lettuce", "Tomato", "Cheese", "Buns"],
      },
      {
        cardId: 5,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/91afd7c438fcf1a3b55b97c2fbaa1cfa",
        title: "Radha Krishna Puri",
        type: "North Indian, Chinese",
        mealDetail: "Authentic North Indian and Chinese dishes.",
        ingredients: ["Paneer", "Capsicum", "Soy sauce", "Rice", "Spices"],
      },
      {
        cardId: 6,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/69a061b7e0f951cef2b4947946f94ec6",
        title: "Box8 - Desi Meals",
        type: "North Indian, Biryani, Thalis",
        mealDetail: "Traditional desi meals with a variety of dishes.",
        ingredients: ["Rice", "Chicken", "Spices", "Vegetables", "Yogurt"],
      },
      {
        cardId: 7,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        title: "Green Delight",
        type: "Salads, Healthy",
        mealDetail: "Fresh and healthy salads with a variety of dressings.",
        ingredients: ["Lettuce", "Tomatoes", "Cucumbers", "Olive oil", "Herbs"],
      },
      {
        cardId: 8,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
        title: "Veggie Feast",
        type: "Vegetarian, Continental",
        mealDetail: "Delicious vegetarian dishes with a continental twist.",
        ingredients: [
          "Paneer",
          "Bell peppers",
          "Tomatoes",
          "Olive oil",
          "Herbs",
        ],
      },
    ],
  },
  "Non-Veg": {
    imageId:
      "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/63178e3e64d503a479f2a2048a474552",
    items: [
      {
        cardId: 3,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/63178e3e64d503a479f2a2048a474552",
        title: "KFC",
        type: "Fried Chicken, Fast Food",
        mealDetail:
          "Crispy and juicy fried chicken with a secret blend of herbs and spices.",
        ingredients: ["Chicken", "Flour", "Spices", "Oil", "Buttermilk"],
      },
      {
        cardId: 4,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/RX_THUMBNAIL/IMAGES/VENDOR/2024/5/20/349e8d46-3138-4b19-96b2-df400a77e285_622197.JPG",
        title: "Domino's Pizza",
        type: "Pizzas, Italian",
        mealDetail:
          "Hot and fresh pizzas with a variety of toppings and crust options.",
        ingredients: [
          "Wheat flour",
          "Cheese",
          "Tomato sauce",
          "Pepperoni",
          "Vegetables",
        ],
      },
      {
        cardId: 5,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/85ccae4e3576f9330af102c46ca85395",
        title: "Grill House",
        type: "Grilled, Barbeque",
        mealDetail: "Grilled meats and vegetables with a smoky flavor.",
        ingredients: ["Chicken", "Beef", "Vegetables", "Spices", "Grill sauce"],
      },
      {
        cardId: 6,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/noc7ieivirqxtpujhsgl",
        title: "Spicy Chicken",
        type: "Indian, Fast Food",
        mealDetail: "Spicy and flavorful chicken dishes with Indian spices.",
        ingredients: ["Chicken", "Spices", "Oil", "Yogurt", "Herbs"],
      },
      {
        cardId: 7,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        title: "Chicken Delight",
        type: "Chicken, Indian",
        mealDetail: "Delicious chicken dishes with Indian spices.",
        ingredients: ["Chicken", "Yogurt", "Spices", "Onions", "Tomatoes"],
      },
    ],
  },
  Curry: {
    imageId:
      "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/69a061b7e0f951cef2b4947946f94ec6",
    items: [
      {
        cardId: 5,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/RX_THUMBNAIL/IMAGES/VENDOR/2024/4/1/b5c7e325-a2b3-4334-b104-0b20df81dd93_23728.JPG",
        title: "Curry House",
        type: "Indian, Curry",
        mealDetail:
          "A variety of Indian curries with rich and flavorful gravies.",
        ingredients: ["Spices", "Tomatoes", "Onions", "Yogurt", "Herbs"],
      },
      {
        cardId: 6,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/e33e1d3ba7d6b2bb0d45e1001b731fcf",
        title: "Veg Curry",
        type: "Vegetarian, Indian",
        mealDetail: "Delicious vegetarian curries with rich flavors.",
        ingredients: ["Paneer", "Tomatoes", "Onions", "Spices", "Herbs"],
      },
      {
        cardId: 7,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/RX_THUMBNAIL/IMAGES/VENDOR/2024/5/20/349e8d46-3138-4b19-96b2-df400a77e285_622197.JPG",
        title: "Chicken Curry",
        type: "Indian, Curry",
        mealDetail: "Spicy and flavorful chicken curry with a rich gravy.",
        ingredients: [
          "Chicken",
          "Tomatoes",
          "Onions",
          "Spices",
          "Coconut milk",
        ],
      },
      {
        cardId: 8,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/6e44fd7f1e5cd9967edfe47c10247671",
        title: "Fish Curry",
        type: "Indian, Seafood",
        mealDetail:
          "Traditional fish curry with aromatic spices and a tangy gravy.",
        ingredients: [
          "Fish",
          "Spices",
          "Coconut milk",
          "Tamarind",
          "Curry leaves",
        ],
      },
    ],
  },
  Lunch: {
    imageId:
      "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
    items: [
      {
        cardId: 7,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/RX_THUMBNAIL/IMAGES/VENDOR/2024/5/19/741d7d41-1341-4358-b6e0-cc22b8e82f9a_750391.JPG",
        title: "Lunch Box",
        type: "Indian, Meals",
        mealDetail:
          "Convenient and nutritious lunch options with balanced meals.",
        ingredients: ["Rice", "Dal", "Vegetables", "Roti", "Salad"],
      },
      {
        cardId: 8,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/6e44fd7f1e5cd9967edfe47c10247671",
        title: "Healthy Lunch",
        type: "Healthy, Indian",
        mealDetail: "Wholesome and healthy lunch options for a balanced diet.",
        ingredients: [
          "Quinoa",
          "Grilled chicken",
          "Vegetables",
          "Olive oil",
          "Herbs",
        ],
      },
      {
        cardId: 9,
        imageId:
          "https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/490629b70f89da8a5b93fc199ece335e",
        title: "Quick Lunch",
        type: "Indian, Fast Food",
        mealDetail: "Quick and tasty lunch options for busy schedules.",
        ingredients: ["Rice", "Paneer", "Vegetables", "Spices", "Chutneys"],
      },
    ],
  },
};

letnewarrobjupdate = "https://mocki.io/v1/d072f4ff-f78a-4bb1-ba68-29792825b177";
letnewarrobjupdate2 ="https://mocki.io/v1/8a0973db-5867-44e1-ac03-daecca507a45";
